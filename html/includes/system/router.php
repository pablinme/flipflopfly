...
class Router
{
    ...
    private static function scan($force = FALSE)
    {
                ...
                foreach ($routes as $route)
                {
                    ...
                    switch ($method)
                    {
                        case self::ROUTE_STATIC:
                            if (URI_BACKBONE === $pattern)
                            {
                                $ctrl = $replacement;
                            }
                        break;
                        case self::ROUTE_PCRE:
                            if (preg_match($pattern, URI_BACKBONE))
                            {
                                $ctrl = preg_replace($pattern, $replacement, URI_BACKBONE);
                            }
                        break;
                        ...
                        case self::REDIRECT_PCRE:
                            if (preg_match($pattern, URI_BACKBONE))
                            {
                                $redirect = preg_replace($pattern, $replacement, URI_BACKBONE);
                            }
                        break;
                    }
        ...
        //if(!$foud) { throw new Exception('Wrong page!'); }
    }
     ...
     * @return string
     */
    public static function start($scan = FALSE)
    {
        self::scan($scan);
        return self::$ctrl;
    }
